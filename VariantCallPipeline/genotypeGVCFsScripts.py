...
"""
## previously used as job dependency #BSUB -w "done(gDB%s)"
...
for chrm in list(range(1,23)) + ['X', 'Y']: #list(range(1,21)) + ['X', 'Y']: #list(range(1,21)) + ['X', 'Y']:
    ...
    for regidx, start in enumerate(range(chromL)[::regionSize]):
        ...
        print ("Job %s previously failed, resubmitting" % region)        
        scriptfile = '/scratch/GenomePipe/GenotypeGVCFs/bsub_%s.sh' % region
